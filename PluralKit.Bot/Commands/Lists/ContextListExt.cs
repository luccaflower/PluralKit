...
namespace PluralKit.Bot
{
    public static class ContextListExt
    {
        ...
        public static async Task RenderSystemReminderList(this Context ctx, IDatabase db, SystemId system, string embedTitle, string color, bool showSeen) {
            var reminders = await db.Execute(conn => showSeen ? conn.QuerySystemReminders(system) : conn.QuerySystemReminders(system, false)).ToListAsync();
            var itemsPerPage = 25;
